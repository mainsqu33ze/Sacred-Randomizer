...
import argparse
import yaml
from fe8.randomizer import apply_config
...
Examples:
  ...
  fe8_randomizer.py ROM.gba -c config.yaml --seed 42 --dump > dump.txt
    ...
    if args.dump:
        from fe8.fe8rom import ROM, CharacterData, PID
        rom = ROM(args.rom)
        ...
        print("  # When true, each slot keeps its original level/base stats/weapon ranks;")
        print("  # the identity moving into that slot inherits the slot's bases.")
        print("  # When false, bases travel with the character identity (full swap).")
        print("  keep_slot_bases: true")
        print("")
        print("# --- Growth Rates ---")
        ...
        print("  devil: 5")
        print("  stone: 3")
        print("  # Brave (bit 5) and reaver (bit 8) — independent per-item % chance in attributes field")
        print("  brave: 5")
        print("  reaver: 3")
        print("")
        print("# --- Affinities ---")
