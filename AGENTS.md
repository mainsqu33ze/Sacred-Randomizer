...
- `randomizer.py` — `apply_config()` orchestrates in order: class → growths → base stats → **promo gain sync** → affinity → weapon stats → weapon effects → promotion items → unit defs → enemy randomize → unit items
