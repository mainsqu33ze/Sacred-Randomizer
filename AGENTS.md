...
- **Portrait palettes** (`portrait_palettes: false`): When enabled during class randomization, generates unique palette sets for each randomized character by mapping their original character portrait colors onto their new class's palette template using weighted RGB color distance (2ΔR² + 4ΔG² + 3ΔB²). The PLAYER sub-palette from the character's original palette set is used as the source; the new class's template PLAYER palette has each of its 16 colors replaced with the nearest color from the source set. ENEMY/NPC/OTHER/LINK sub-palettes are copied from the class template unchanged. New palette data is stored in free ROM space at 0x08FE3090 with new table entries at 0x08EF86D4+. Uses `lz77_compress`/`lz77_decompress` helpers in fe8rom.py.
...
- **Chest items** (`_scan_chest_items`): Scans section[2] (`locationBasedEvents`) of each chapter's event data for type-0x07 CHES entries (`EvCheck07_CHES` struct: `unk0+givenItem+givenMoney+x+y+cmdId`, 12 bytes). Ony entries with `cmdId=0x14` (TILE_COMMAND_CHEST) and `givenItem > 0` (excluding gold=0x77) are returned. 40 non-gold chest items exist across 17 chapter variants. Gold chests (7 total, entries with `givenItem=0x77`) are skipped — gold amount is in `givenMoney`.
- **Loot randomization** (`loot_randomization`): `mode: random | shuffle`. Scans GiveItem (`0x1E`) commands via `_scan_loot_events()` AND chest items via `_scan_chest_items()`, both using the same loot pool and mode. Runs after promotion item conversion.
...
## Chest items
- All chest items are in **type-0x07 CHES entries** in `locationBasedEvents` (event data section[2]). No chests use type-0x05 LOCA entries or weighted-random script-based item tables.
- `EvCheck07_CHES` struct (12 bytes per entry): `[unk0(u32)=0x00000007, givenItem(u16), givenMoney(u16), x(u8), y(u8), cmdId(u16)=0x14]`. The list is terminated by an all-zero entry.
- **Gold chests** use `givenItem=0x77` (ITEM_GOLD) with the gold amount in `givenMoney`. These are excluded from randomization.
- **Non-gold chests** (40 across 17 chapters/route-variants) are randomized using the same loot pool as GiveItem events. Both `random` and `shuffle` modes are supported, controlled by the same `loot_randomization.mode` config key.
- Chests are separate from GiveItem events — chest items are patched at offset `entry_off + 4` (the `givenItem` u16 field), not via `40 0A 00 00` event commands.
- `randomize_chest()` is called from `apply_config()` immediately after `randomize_loot()`.
