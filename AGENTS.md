...
- **Item pools** (`build_weapon_pools`): filters `stored_id == item_id`, `weapon_type ≤ 7`, `uses > 0`, `range > 0`. Excludes `MONSTER_BLOCKED_ITEM_IDS` (16), `BALLISTA_ITEM_IDS` (Iron Ballista 0x23, Killer Ballista 0x24, Ballista 0x25), and `STORY_EXCLUSIVE_ITEM_IDS` (Rapier 0x09, Reginleif 0x78, Sieglinde 0x85, Siegmund 0x92). Staves (type 4) included with `might == 0`; non-staves need `might > 0`. Ballista also excluded from loot pools and weapon stat/effect randomization — they crash when wielded in normal combat.
